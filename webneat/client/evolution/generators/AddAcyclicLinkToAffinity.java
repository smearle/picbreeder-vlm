...
public class AddAcyclicLinkToAffinity extends AbstractMutator {
	private final String affinity;
	
	public AddAcyclicLinkToAffinity(String affinity) {
	...
	public void mutate(Genome offspring) {
		...
		for(Node n : offspring.getNodes())
			if(n.getAffinity().equals(affinity))
					nodes.add(n);
		
		// arbitrarily pick a source node
		// do note allow the outputs as sources since they are output of subnet
		...
		// nodes can be empty if a color network was just created with 0 hidden
		// nodes and we are in the color affinity
		if(nodes.size() == 0)
			return;
