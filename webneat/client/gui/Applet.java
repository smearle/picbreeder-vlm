...
public class Applet extends JApplet implements Initialization, ImageDatabase, SessionEnd {
	...
	public void publish() {
		...
		redirect("editgenome.php?sid="+seriesId+"&pid=0");
	}
	...
	public void publishanonymously()
	{
		...
		redirect("editgenome.php?sid="+seriesId+"&pid=0"+"&GUID="+guid);
		}
	...
	public String getParameter(String name) {
		...
		// resolve the ip now and make sure the server is the same!
		if(name.equals("webservices")) {
			String temp = result;
			String prefix = "http://";
			
			if(temp.startsWith(prefix)) {
				temp = result.substring(prefix.length() + 1);
			
				int delim = temp.indexOf(':');
				if(delim == -1) { // no port given!
					delim = temp.indexOf('/');
					
					if(delim == -1) { // no port AND no path given
						delim = 0;
						temp = "/" + temp;
					}
				}
				
				temp = temp.substring(delim);
			}
			else if(!temp.startsWith("/"))
				temp = "/" + temp;
			
			try {
				java.net.InetAddress address = java.net.InetAddress.getByName(getDocumentBase().getHost());
				result = prefix + address.getHostAddress() + temp;
			}
			catch(java.net.UnknownHostException e) {
				e.printStackTrace();
				// note: returns original parameter since result is not written
			}
		}
