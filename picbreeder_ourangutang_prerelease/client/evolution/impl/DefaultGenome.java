...
class DefaultGenome implements Genome {
	...
	DefaultGenome(Identifier id, String [] in, String [] out, int hiddenCount) {
		...
		for(String input : in)
			inputs.add(GeneticFactoryInstance.get().createNode(input, "in"));
		nodes.addAll(inputs);
		
		for(String output : out)
			outputs.add(GeneticFactoryInstance.get().createNode(output, "out"));
		nodes.addAll(outputs);
		
		for(int i = 0; i < hiddenCount; i++)
			hidden.add(GeneticFactoryInstance.get().createNode("", "hidden"));
		nodes.addAll(hidden);
	...
	public void addLink(Link link) {
		links.add(link);
		if(link.compareTo(links.get(links.size() - 2)) < 0)
			Collections.sort(links);
	}
	
	public void addNode(Node node) {
		nodes.add(node);
		if(node.compareTo(nodes.get(nodes.size() - 2)) < 0)
			Collections.sort(nodes);
	}
