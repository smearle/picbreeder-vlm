...
*   `logs_collaborative/<experiment_name>/`: output of a **single** run
    (`evolve_collaborative.py`; its default `log_dir` is `logs_collaborative`).
*   `sweep_logs/sweep/<run_name>/`: output of a **sweep** run
    (`-m picbreeder_vlm.experiments.sweep`; its default `log_dir` is `sweep_logs`).
    Each run directory contains:
        *   `archive/`: images (PNG, re-rendered from genomes) and genomes (pickle).
    ...
    `sweep_logs/sweep/<run_name>/` so the eval/figure commands find them.)
*   `cross_eval/<sweep_name>/`: Aggregated results (plots, tables, LaTeX).
*   `data/noun_lists/`: Text files containing lists of target nouns for alignment tasks.
*   `initial_populations/`: Seed populations for runs.
*   `human_lineages/`: Renders of the *human* Picbreeder archive, generated by the
    `third-party/fer/src/` scripts and read back by `tools/build_human_*.py`. Gitignored.
*   `figures/`: TikZ sources for the blog figures. The paper lives in Overleaf.
*   `third-party/`: Vendored external code (`webneat/`, `fer/`). Resolve `fer/`
    via `picbreeder_vlm._paths.FER_ROOT`; see `third-party/README.md`.
